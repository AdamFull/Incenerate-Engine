...
//  2022-01-20: Vulkan: Added support for ImTextureID as vk::DescriptorSet. User need to call ImGui_ImplVulkan_AddTexture(). Building for 32-bit targets requires '#define ImTextureID ImU64'. (#914).
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, vk::CommandBuffer command_buffer, vk::Pipeline pipeline)
{
    ...
    if (draw_data->TotalVtxCount > 0)
    {
        ...
        err = v->Device.flushMappedMemoryRanges(2, range);
        check_vk_result(err);
...
bool ImGui_ImplVulkan_CreateFontsTexture(vk::CommandBuffer command_buffer)
{
    ...
    // Upload to Buffer:
    {
        ...
        err = v->Device.flushMappedMemoryRanges(1, range);
        check_vk_result(err);
...
static void ImGui_ImplVulkan_CreatePipeline(vk::Device device, const vk::AllocationCallbacks* allocator, vk::PipelineCache pipelineCache, vk::RenderPass renderPass, vk::SampleCountFlagBits MSAASamples, vk::Pipeline* pipeline, uint32_t subpass)
{
    ...
    color_attachment[0].dstColorBlendFactor = vk::BlendFactor::eConstantAlpha;
...
void ImGui_ImplVulkan_SetMinImageCount(uint32_t min_image_count)
{
    ...
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
    vk::Result err = vkDeviceWaitIdle(v->Device);
    check_vk_result(err);
    ImGui_ImplVulkanH_DestroyAllViewportsRenderBuffers(v->Device, v->Allocator);
...
vk::DescriptorSet ImGui_ImplVulkan_AddTexture(vk::Sampler sampler, vk::ImageView image_view, vk::ImageLayout image_layout)
{
    ...
    vk::DescriptorSet descriptor_set;
    {
        ...
        vk::Result err = vv->Device->allocateDescriptorSets(&alloc_info, &descriptor_set);
        check_vk_result(err);
    }
...
void ImGui_ImplVulkan_RemoveTexture(vk::DescriptorSet descriptor_set)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
...
VkSurfaceFormatKHR ImGui_ImplVulkanH_SelectSurfaceFormat(vk::PhysicalDevice physical_device, VkSurfaceKHR surface, const VkFormat* request_formats, int request_formats_count, VkColorSpaceKHR request_color_space)
{
    IM_ASSERT(g_FunctionsLoaded && "Need to call ImGui_ImplVulkan_LoadFunctions() if IMGUI_IMPL_VULKAN_NO_PROTOTYPES or VK_NO_PROTOTYPES are set!");
...
VkPresentModeKHR ImGui_ImplVulkanH_SelectPresentMode(vk::PhysicalDevice physical_device, VkSurfaceKHR surface, const VkPresentModeKHR* request_modes, int request_modes_count)
{
    IM_ASSERT(g_FunctionsLoaded && "Need to call ImGui_ImplVulkan_LoadFunctions() if IMGUI_IMPL_VULKAN_NO_PROTOTYPES or VK_NO_PROTOTYPES are set!");
...
static void ImGui_ImplVulkan_CreateWindow(ImGuiViewport* viewport)
{
    ...
    // Create surface
    ImGuiPlatformIO& platform_io = ImGui::GetPlatformIO();
    vk::Result err = (vk::Result)platform_io.Platform_CreateVkSurface(viewport, (ImU64)v->Instance, (const void*)v->Allocator, (ImU64*)&wd->Surface);
    check_vk_result(err);
    ...
    const VkFormat requestSurfaceImageFormat[] = { VK_FORMAT_B8G8R8A8_UNORM, vk::Format::eR8G8B8A8Unorm, VK_FORMAT_B8G8R8_UNORM, VK_FORMAT_R8G8B8_UNORM };
